...
Результат:
- папка `reference/legacy-js/` як архівний snapshot, не runtime dependency
...
- reaction counting
- `1 ❤️ -> Друк`
- пересилання в `ЗАМОВЛЕННЯ` після `1 ❤️`
- `👍 -> Пакування` як pilot-disabled stage у поточному релізі
- no rollback by default
- duplicate protection

Критерій завершення:
- реакції не викликають фліп-флоп статусів;
- один `PRINT` stage викликає рівно одну зміну статусу і одне пересилання.
